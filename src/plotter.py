...
class Plotter():
    ...
    def plot_pulsatile_dynamics(self, path_figs, Tstop):
        ...
        for n in range(int(100*1e3), int(103*1e3)+1, 10):
            ...
            # append data to lists
            j_ins_slow.append(j_in(point))
            phi_es_slow.append(phi_e(point)*1000)       # convert to mV
            K_es_slow.append(K_e(point))
            dalpha_es_slow.append(alpha_e_diff)
        ...
        for n in range(int(100*1e3), int(160*1e3)+1, int(5e2)):
            ...
            # append data to lists
            j_ins_ultraslow.append(j_in(point))
        ...
        for n in range(Tstop+1):
            ...
            # append data to lists
            j_ins_constant.append(j_in(point))
            phi_es_constant.append(phi_e(point)*1000)   # convert to mV
            K_es_constant.append(K_e(point))
            dalpha_es_constant.append(alpha_e_diff)

        # create plot
        fig = plt.figure(figsize=(15*fs, 20*fs))
        ax = plt.gca()

        ax1 = fig.add_subplot(4, 3, 1, ylim=[-0.05, 1.0])
        plt.ylabel(r'$j\mathrm{^K_{input}}$($\mu$mol/(m$^2$s))', fontsize=fosi)
        ...
        ax5 = fig.add_subplot(4, 3, 5, ylim=[-3, 0.1])
        plt.plot(phi_es_ultraslow, color=colors[2], linewidth=lw)

        ax6 = fig.add_subplot(4, 3, 6, xlim=xlim_T, ylim=[-3, 0.1])
        plt.plot(phi_es_constant, color=colors[3], linewidth=lw)

        ax7 = fig.add_subplot(4, 3, 7, ylim=[-0.2, 7.5])
        plt.ylabel(r'$\Delta [\mathrm{K}^+]_\mathrm{e}$ (mM)', fontsize=fosi)
        ax7.set_yticks([0, 2.5, 5, 7.5])
        plt.plot(np.array(K_es_slow)-K_es_constant[0], color=colors[0], linewidth=lw, zorder=10)

        ax8 = fig.add_subplot(4, 3, 8, ylim=[-0.2, 7.5])
        ax8.set_yticks([0, 2.5, 5, 7.5])
        plt.plot(np.array(K_es_ultraslow)-K_es_constant[0], color=colors[2], linewidth=lw)

        ax9 = fig.add_subplot(4, 3, 9, xlim=xlim_T, ylim=[-0.2, 7.5])
        ax9.set_yticks([0, 2.5, 5, 7.5])
        plt.plot(np.array(K_es_constant)-K_es_constant[0], color=colors[3], linewidth=lw)

        ax10 = fig.add_subplot(4, 3, 10, ylim=[-10, 0.2])
        plt.ylabel(r'$\Delta \alpha_\mathrm{e}$ (\%) ', fontsize=fosi)
        plt.xlabel(r'time (s)', fontsize=fosi)
        plt.plot(dalpha_es_slow, color=colors[0], linewidth=lw, zorder=10)

        ax11 = fig.add_subplot(4, 3, 11, ylim=[-10, 0.2])
        plt.xlabel(r'time (s)', fontsize=fosi)
        plt.plot(dalpha_es_ultraslow, color=colors[2], linewidth=lw)

        ax12 = fig.add_subplot(4, 3, 12, xlim=xlim_T, ylim=[-10, 0.2])
        plt.xlabel(r'time (s)', fontsize=fosi)
        plt.plot(dalpha_es_constant, color=colors[3], linewidth=lw)
        ...
        # save figure to file
        fname_res = path_figs + 'Figure7'
        plt.savefig(fname_res + '.pdf', format='pdf')
        plt.close()

        return

    def plot_pulsatile_velocities(self, path_figs, Tstop):
        ...
        # project to function space
        u_i_tot_slow_I = self.project_to_function_space(alpha_i*u_i_tot_*1.0e6*60) # convert to um/min
        ...
        # project to function space
        u_i_tot_slow_III = self.project_to_function_space(alpha_i*u_i_tot_*1.0e6*60) # convert to um/min
        ...
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        df.plot(u_i_tot_slow_I, label='$t=100.25$ s', color=t0, linewidth=lw)
        df.plot(u_i_tot_slow_II, label='$t=100.5$ s', color=t1, linewidth=lw)
        df.plot(u_i_tot_slow_III, label='$t=100.75$ s', color=t2, linewidth=lw)
        ...
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        df.plot(u_i_tot_constant, label=r'$t=100$ s', color=t2, linewidth=lw)
        ...
        for n in range(Tstop+1):
            ...
            max_us_constant.append(float(max_u_i))
        ...
        for n in range(int(100*1e3), int(103*1e3)+1, 10):
            ...
            max_us_slow.append(float(max_u_i))
            ...
            j_in = self.project_to_function_space(j_in_*1e6)    # convert to umol/(m^2s)
            t += 10e-3
            j_ins_slow.append(j_in(point_time))
        ...
        ax4t = ax4.twinx()
        plt.plot(j_ins_slow, color='lightgrey', linewidth=lw)
        plt.ylim([-0.015, 1.0])
        plt.plot(25, max(j_ins_slow), 'o', color=t0, markersize=8, markeredgecolor='dimgrey')
        plt.plot(50, max(j_ins_slow)/2, 'o', color=t1, markersize=8, markeredgecolor='dimgrey')
        plt.plot(75, min(j_ins_slow), 'o', color=t2, markersize=8, markeredgecolor='dimgrey')
        ax4.set_zorder(10)
        ...
        ax5t = ax5.twinx()
        plt.plot(j_ins_ultraslow, color='lightgrey', linewidth=lw)
        plt.ylim([-0.015, 1.0])
        plt.plot(10, max(j_ins_ultraslow), 'o', color=t0, markersize=8, markeredgecolor='dimgrey')
        plt.plot(20, max(j_ins_ultraslow)/2, 'o', color=t1, markersize=8, markeredgecolor='dimgrey')
        plt.plot(30, min(j_ins_ultraslow), 'o', color=t2, markersize=8, markeredgecolor='dimgrey')
        ax5.set_zorder(10)
        ...
        ax6.plot(100, max(max_us_constant[100:150]), 'o', color=t2, markersize=8, markeredgecolor='dimgrey')
        ...
        for ax in [ax4t, ax5t, ax6t]:
            ax.spines['top'].set_visible(False)

        # save figure to file
        fname_res = path_figs + 'Figure8'
        plt.savefig(fname_res + '.pdf', format='pdf')
        plt.close()
