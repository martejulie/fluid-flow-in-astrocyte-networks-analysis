...
class Plotter():
    ...
    def set_datafile_I(self, path_data):
        ...
        hdf5_PDE_I = HDF5File(MPI.comm_world, self.h5_fname_PDE_I, 'r')
    ...
    def set_datafile_II(self, path_data):
        ...
        # read data file
        self.hdf5_II = HDF5File(MPI.comm_world, self.h5_fname_PDE_II, 'r')
    ...
    def set_datafile_III(self, path_data):
        ...
        # read data file
        self.hdf5_III = HDF5File(MPI.comm_world, self.h5_fname_PDE_III, 'r')
    ...
    def set_datafile_0(self, path_data):
        ...
        # read data file
        self.hdf5_0 = HDF5File(MPI.comm_world, self.h5_fname_PDE_0, 'r')
    ...
    def read_from_file_I(self, n, i):
        ...
        CG1 = FiniteElement('CG', self.mesh_PDE.ufl_cell(), 1)
        R = FiniteElement("R", self.mesh_PDE.ufl_cell(), 0) # element for Lagrange multiplier
        e = [CG1]*(N_comparts - 1) + [CG1]*(N_unknowns - (N_comparts - 1))
        W = FunctionSpace(self.mesh_PDE, MixedElement(e + [R]))
        u = Function(W)

        V_CG1 = FunctionSpace(self.mesh_PDE, CG1)
        f = Function(V_CG1)
    ...
    def project_to_function_space(self, u):
        ...
        CG1 = FiniteElement('CG', self.mesh_PDE.ufl_cell(), 1)
        V = FunctionSpace(self.mesh_PDE, CG1)
        f = project(u, V)
    ...
    def plot_flow_dynamics(self, path_figs, n):
        ...
        u_i = - alpha_i*kappa[0]*grad(p_i)*1.0e6*60 # convert to um/min

        # ECS fluid velocities
        u_e = - alpha_e*kappa[1]*grad(p_e)*1.0e6*60 # convert to um/min
        ...
        plot(dOsm_e, label=r'ECS', color=colors[2], linewidth=lw)
        plot(dOsm_i, label=r'ICS', color='k', linewidth=lw, linestyle='dotted')
        ...
        plot(dPi_m, label=r'$\Delta(\Pi_\mathrm{i}-\Pi_\mathrm{e})$', color=c1, linewidth=lw)
        plot(dp_m, label=r'$\Delta(p_\mathrm{i}-p_\mathrm{e})$', color=c2, \
        ...
        plot(w_m, color=c0, linewidth=lw, linestyle=(0, (5, 1)))
        
        ax4 = fig.add_subplot(2,3,4, xlim=xlim, ylim=[-100, 30])
        plt.xticks(xticks, xticklabels)
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.ylabel(r'$\Delta p$ (Pa)')
        plot(dp_e, color=colors[2], linewidth=lw)
        plot(dp_i, color='k', linewidth=lw, linestyle='dotted')
        
        ax5 = fig.add_subplot(2,3,5, xlim=xlim, ylim=[-0.35, 0.35])
        plt.xticks(xticks, xticklabels)
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.ylabel(r'$\alpha u$ ($\mu$m/min)')
        plot(u_e, color=colors[2], linewidth=lw) 
        plot(u_i, color='k', linewidth=lw, linestyle='dotted') 
    ...
    def plot_osmotic_pressures_and_water_potentials(self, path_figs, n):
        ...
        alpha_e_0 = 0.6 - alpha_i_0
        tau_0 = K_m*(alpha_i_0 - float(self.model.alpha_i_init))
        p_i_0 = p_e_0 + tau_0 + p_m_init
        ...
        tau_M1 = K_m*(alpha_i_M1 - alpha_i_init)
        p_i_M1 = p_e_M1 + tau_M1 + p_m_init
        ...
        tau_M2 = K_m*(alpha_i_M2 - float(self.model.alpha_i_init))
        p_i_M2 = p_e_M2 + tau_M2 + p_m_init
        ...
        dPi_m_M0 = (dPi_i_M0 - dPi_e_M0)/1000 # convert to kPa
        dPi_m_M1 = (dPi_i_M1 - dPi_e_M1)/1000
        dPi_m_M2 = (dPi_i_M2 - dPi_e_M2)/1000

        # water potentials (kPa)
        dPsi_M0 = dPi_e_M0/1000 # convert to kPa
        ...
        plt.xticks(xticks, xticklabels)
        plot(dOsm_e_M0, color=colors[3], linewidth=lw, label='M0')
        plot(dOsm_e_M1, color=colors[0], linewidth=lw, label='M1')
        plot(dOsm_e_M2, color=colors[2], linewidth=lw, label='M2')

        ax2 = fig.add_subplot(2,3,2, xlim=xlim, ylim=[-40, 15])
        plt.ylabel(r'$\Delta$O ICS (mM)', fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        plot(dOsm_i_M0, color=colors[3], linewidth=lw)
        plot(dOsm_i_M1, color=colors[0], linewidth=lw)
        plot(dOsm_i_M2, color=colors[2], linewidth=lw)
        
        ax3 = fig.add_subplot(2,3,3, xlim=xlim, ylim=[-130, 10])
        plt.ylabel(r'$\Delta (\Pi_\mathrm{i} - \Pi_\mathrm{e})$ (kPa)', fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        plot(dPi_m_M0, color=colors[3], linewidth=lw)
        plot(dPi_m_M1, color=colors[0], linewidth=lw)
        plot(dPi_m_M2, color=colors[2], linewidth=lw)
        
        ax4 = fig.add_subplot(2,3,4, xlim=xlim, ylim=[-40, 100])
        plt.ylabel(r'$\Delta \Pi_\mathrm{e}$ (kPa)', fontsize=fosi)
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        plot(dPi_e_M0, color=colors[3], linewidth=lw)
        plot(dPi_e_M1, color=colors[0], linewidth=lw)
        plot(dPi_e_M2, color=colors[2], linewidth=lw)
        ...
        plot(dp_e_M1, color=colors[0], linewidth=lw)
        plot(dp_e_M2, color=colors[2], linewidth=lw)

        ax6 = fig.add_subplot(2,3,6, xlim=xlim, ylim=[-40, 100]) 
        plt.ylabel(r'$\Delta (\Pi_\mathrm{e} + p_\mathrm{e})$ (kPa)', fontsize=fosi)
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        plot(dPsi_zero, color=colors[3], linewidth=lw)
        plot(dPsi_M1, color=colors[0], linewidth=lw)
        plot(dPsi_M2, color=colors[2], linewidth=lw)

        if self.verbose == True:
            print('max O_e decrease M0:', min(dOsm_e_zero_flow.vector()[:]))
            print('max O_e decrease M1:', min(dOsm_e_M1.vector()[:]))
            print('max O_e decrease M2:', min(dOsm_e_M2.vector()[:]))
            print('max O_i increase M0:', max(dOsm_i_zero_flow.vector()[:]))
            print('max O_i decrease M1:', min(dOsm_i_M1.vector()[:]))
            print('max O_i increase M2:', max(dOsm_i_M2.vector()[:]))
            print('max Pi_m decrease M0:', min(dPi_m_zero_flow.vector()[:]))
            print('max Pi_m decrease M1:', min(dPi_m_M1.vector()[:]))
            print('max Pi_m decrease M2:', min(dPi_m_M2.vector()[:]))
            print('max Pi_e increase M0', max(dPi_e_zero_flow.vector()[:]))
            ...
            print('max Psi increase M0', max(dPsi_zero.vector()[:]))
    ...
    def plot_M2_and_M3_fluid_velocities(self, path_figs, n):
        ...
        Na_i = self.read_from_file_II(n, 1)
        Na_e = self.read_from_file_II(n, 2)
        K_i = self.read_from_file_II(n, 3)
        K_e = self.read_from_file_IJ(n, 4)
        Cl_i = self.read_from_file_II(n, 5)
        Cl_e = self.read_from_file_II(n, 6)
        ...
        # ICS fluid velocities
        u_i_hyd_ = - kappa[0]*grad(p_i)
        u_i_osm_ = kappa[0]*R*temperature*grad(a_i/alpha_i)
        u_i_tot_ = u_i_hyd_[0] + u_i_osm_[0]

        # ECS fluid velocities
        u_e_hyd_ = - kappa[1]*grad(p_e)
        u_e_tot_ = u_e_hyd_[0]
        ...
        plt.xticks(xticks, xticklabels)
        plot(u_i_osm, color=c5, linewidth=lw)
        plot(u_i_hyd, color=c3, linewidth=lw, linestyle='dotted')
        plot(u_i_tot, color='k', linewidth=lw, linestyle='dashed')
        
        ax2 = fig.add_subplot(2,2,2, xlim=xlim, ylim=[-90, 90])
        plt.title(r'(M2)', fontsize=fosi)
        plt.ylabel(r'$\alpha_\mathrm{e} u_\mathrm{e}$ ($\mu$m/min)', fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        plot(u_e_hyd, color=c3, linewidth=lw, linestyle='dotted')
        plot(u_e_tot, color='k', linewidth=lw, linestyle='dashed')

        # get data M3
        alpha_i = self.read_from_file_III(n, 0)
        alpha_i = self.read_from_file_III(n, 0)
        Na_i = self.read_from_file_III(n, 1)
        Na_e = self.read_from_file_III(n, 2)
        K_i = self.read_from_file_III(n, 3)
        K_e = self.read_from_file_III(n, 4)
        Cl_i = self.read_from_file_III(n, 5)
        Cl_e = self.read_from_file_III(n, 6)
        ...
        # ECS fluid velocities
        u_e_hyd_ = - kappa[1]*grad(p_e)
        u_e_eof_ = - eps_r*eps_zero*zeta*grad(phi_e)/mu
        ...
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        plot(u_i_osm, color=c5, label='osmotic', linewidth=lw)
        plot(u_i_hyd, color=c3, linewidth=lw, linestyle='dotted')
        plot(u_i_tot, color='k', linewidth=lw, linestyle='dashed')
        ...
        plt.xlabel(xlabel_x, fontsize=fosi)
        plt.xticks(xticks, xticklabels)
        plot(u_e_hyd, label='hydrostatic', color=c3, linewidth=lw, linestyle='dotted')
        plot(u_e_eof, label='electro-osmotic', color=c4, linewidth=lw)
        plot(u_e_tot, label='total', color='k', linewidth=lw, linestyle='dashed')
    ...
    def plot_ion_fluxes(self, path_figs, n):
        ...
        u_i_ = - kappa[0]*grad(p_i_)
        u_e_ = - kappa[1]*grad(p_e_)

        # axial ion fluxes
        j_e_Na_ = - (D[0]/lambda_e**2)*(grad(Na_e_) + z_Na*F*Na_e_/(R*temperature)*grad(phi_e_)) + u_e_*Na_e_
        j_e_K_ = - (D[1]/lambda_e**2)*(grad(K_e_) + z_K*F*K_e_/(R*temperature)*grad(phi_e_)) + u_e_*K_e_
        j_e_Cl_ = - (D[2]/lambda_e**2)*(grad(Cl_e_) + z_Cl*F*Cl_e_/(R*temperature)*grad(phi_e_)) + u_e_*Cl_e_
        j_i_Na_ = - (D[0]/lambda_i**2)*(grad(Na_i_) + z_Na*F*Na_i_/(R*temperature)*grad(phi_i_)) + u_i_*Na_i_
        j_i_K_ = - (D[1]/lambda_i**2)*(grad(K_i_) + z_K*F*K_i_/(R*temperature)*grad(phi_i_)) + u_i_*K_i_
        j_i_Cl_ = - (D[2]/lambda_i**2)*(grad(Cl_i_) + z_Cl*F*Cl_i_/(R*temperature)*grad(phi_i_)) + u_i_*Cl_i_

        j_i_diff_K_ = - (D[1]/lambda_i**2)*grad(K_i_)
        j_i_field_K_ = - (D[1]/lambda_i**2)*(z_K*F*K_i_/(R*temperature)*grad(phi_i_))
        j_i_flow_K_ = u_i_*K_i_
        
        j_e_diff_K_ = - (D[1]/lambda_e**2)*grad(K_e_)
        j_e_field_K_ = - (D[1]/lambda_e**2)*(z_K*F*K_e_/(R*temperature)*grad(phi_e_))
        j_e_flow_K_ = u_e_*K_e_

        j_i_diff_Na_ = - (D[0]/lambda_i**2)*grad(Na_i_)
        j_i_field_Na_ = - (D[0]/lambda_i**2)*(z_Na*F*Na_i_/(R*temperature)*grad(phi_i_))
        j_i_flow_Na_ = u_i_*Na_i_
        
        j_e_diff_Na_ = - (D[0]/lambda_e**2)*grad(Na_e_)
        j_e_field_Na_ = - (D[0]/lambda_e**2)*(z_Na*F*Na_e_/(R*temperature)*grad(phi_e_))
        j_e_flow_Na_ = u_e_*Na_e_
        
        j_i_diff_Cl_ = - (D[2]/lambda_i**2)*grad(Cl_i_)
        j_i_field_Cl_ = - (D[2]/lambda_i**2)*(z_Cl*F*Cl_i_/(R*temperature)*grad(phi_i_))
        j_i_flow_Cl_ = u_i_*Cl_i_
        
        j_e_diff_Cl_ = - (D[2]/lambda_e**2)*grad(Cl_e_)
        j_e_field_Cl_ = - (D[2]/lambda_e**2)*(z_Cl*F*Cl_e_/(R*temperature)*grad(phi_e_))
        ...
        F_K_i = j_i_flow_K.vector()[max_index_j_i_K] / (j_i_diff_K.vector()[max_index_j_i_K] + j_i_field_K.vector()[max_index_j_i_K])
        ...
        F_K_e = j_e_flow_K.vector()[max_index_j_e_K] / (j_e_diff_K.vector()[max_index_j_e_K] + j_e_field_K.vector()[max_index_j_e_K])
        ...
        F_Na_i = j_i_flow_Na.vector()[max_index_j_i_Na] / (j_i_diff_Na.vector()[max_index_j_i_Na] + j_i_field_Na.vector()[max_index_j_i_Na])
        ...
        F_Na_e = j_e_flow_Na.vector()[max_index_j_e_Na] / (j_e_diff_Na.vector()[max_index_j_e_Na] + j_e_field_Na.vector()[max_index_j_e_Na])
        ...
        F_Cl_i = j_i_flow_Cl.vector()[max_index_j_i_Cl] / (j_i_diff_Cl.vector()[max_index_j_i_Cl] + j_i_field_Cl.vector()[max_index_j_i_Cl])
        ...
        F_Cl_e = j_e_flow_Cl.vector()[max_index_j_e_Cl] / (j_e_diff_Cl.vector()[max_index_j_e_Cl] + j_e_field_Cl.vector()[max_index_j_e_Cl])
        ...
        plot(j_e_diff_K, label='diffusion', color=c3, linewidth=3)
        plot(j_e_field_K, label='electric drift', color=c4, linewidth=3)
        plot(j_e_flow_K, label='advection', color=c5, linewidth=3)
        plot(j_e_K, color='dimgrey', linewidth=3, linestyle='dashed')
        ...
        plot(j_e_diff_Na, color=c3, linewidth=3)
        plot(j_e_field_Na, color=c4, linewidth=3)
        plot(j_e_flow_Na, color=c5, linewidth=3)
        plot(j_e_Na, color='dimgrey', linewidth=3, linestyle='dashed')
        ...
        plot(j_e_diff_Cl, color=c3, linewidth=3)
        plot(j_e_field_Cl, color=c4, linewidth=3)
        plot(j_e_flow_Cl, color=c5, linewidth=3)
        plot(j_e_Cl, color='dimgrey', linewidth=3, linestyle='dashed')
        ...
        plot(j_i_diff_K, color=c3, linewidth=3)
        plot(j_i_field_K, color=c4, linewidth=3)
        plot(j_i_flow_K, color=c5, linewidth=3)
        plot(j_i_K, color='dimgrey', label='total', linewidth=3, linestyle='dashed')
        ...
        plt.xticks(xticks, xticklabels)
        plt.title('ICS Na$^+$ (M1)')
        plot(j_i_diff_Na, color=c3, linewidth=3)
        plot(j_i_field_Na, color=c4, linewidth=3)
        plot(j_i_flow_Na, color=c5, linewidth=3)
        plot(j_i_Na, color='dimgrey', linewidth=3, linestyle='dashed')
        ...
        plt.xticks(xticks, xticklabels)
        plt.title('ICS Cl$^-$ (M1)')
        plot(j_i_diff_Cl, color=c3, linewidth=3)
        plot(j_i_field_Cl, color=c4, linewidth=3)
        plot(j_i_flow_Cl, color=c5, linewidth=3)
        plot(j_i_Cl, color='dimgrey', linewidth=3, linestyle='dashed')
        ...
        # intra- and extracellular fluid velocities
        u_i_ = - kappa[0]*(grad(p_i_) - R*temperature*grad(a_i_/alpha_i_)) 
        u_e_ = - kappa[1]*grad(p_e_) - eps_r*eps_zero*zeta*grad(phi_e_)/mu
        ...
        plt.ylabel(r'$\alpha_\mathrm{e}j_\mathrm{e}$ ($\mu$mol/(m$^2$s)')
        plot(j_e_diff_K, color=c3, linewidth=3)
        plot(j_e_field_K, color=c4, linewidth=3)
        plot(j_e_flow_K, color=c5, linewidth=3)
        plot(j_e_K, color='dimgrey', linewidth=3, linestyle='dashed')
        ax7.text(0.03, 0.13, r'F\textsubscript{diff} = ' + str(abs(round(F_K_e_diff, 3))), horizontalalignment='left', verticalalignment='center', transform=ax7.transAxes, size=16)
        ax7.text(0.03, 0.05, r'F\textsubscript{drift} = ' + str(abs(round(F_K_e_drift, 3))), horizontalalignment='left', verticalalignment='center', transform=ax7.transAxes, size=16)
        ...
        plot(j_e_flow_Na, color=c5, linewidth=3)
        plot(j_e_Na, color='dimgrey', linewidth=3, linestyle='dashed')
        ax8.text(0.03, 0.13, r'F\textsubscript{diff} = ' + str(abs(round(F_Na_e_diff, 3))), horizontalalignment='left', verticalalignment='center', transform=ax8.transAxes, size=16)
        ax8.text(0.03, 0.05, r'F\textsubscript{drift} = ' + str(abs(round(F_Na_e_drift, 3))), horizontalalignment='left', verticalalignment='center', transform=ax8.transAxes, size=16)
        ...
        plot(j_e_flow_Cl, color=c5, linewidth=3)
        plot(j_e_Cl, color='dimgrey', linewidth=3, linestyle='dashed')
        ax9.text(0.03, 0.13, r'F\textsubscript{diff} = ' + str(abs(round(F_Cl_e_diff, 3))), horizontalalignment='left', verticalalignment='center', transform=ax9.transAxes, size=16)
        ax9.text(0.03, 0.05, r'F\textsubscript{drift} = ' + str(abs(round(F_Cl_e_drift, 3))), horizontalalignment='left', verticalalignment='center', transform=ax9.transAxes, size=16)
        ...
        plot(j_i_field_K, color=c4, linewidth=3)
        plot(j_i_flow_K, color=c5, linewidth=3)
        plot(j_i_K, color='dimgrey', linewidth=3, linestyle='dashed')
        ax10.text(0.03, 0.13, r'F\textsubscript{diff} = ' + str(abs(round(F_K_i_diff, 3))), horizontalalignment='left', verticalalignment='center', transform=ax10.transAxes, size=16)
        ax10.text(0.03, 0.05, r'F\textsubscript{drift} = ' + str(abs(round(F_K_i_drift, 3))), horizontalalignment='left', verticalalignment='center', transform=ax10.transAxes, size=16)
        ...
        plot(j_i_flow_Na, color=c5, linewidth=3)
        plot(j_i_Na, color='dimgrey', linewidth=3, linestyle='dashed')
        ax11.text(0.03, 0.13, r'F\textsubscript{diff} = ' + str(abs(round(F_Na_i_diff, 3))), horizontalalignment='left', verticalalignment='center', transform=ax11.transAxes, size=16)
        ax11.text(0.03, 0.05, r'F\textsubscript{drift} = ' + str(abs(round(F_Na_i_drift, 3))), horizontalalignment='left', verticalalignment='center', transform=ax11.transAxes, size=16)
        ...
        plot(j_i_flow_Cl, color=c5, linewidth=3)
        plot(j_i_Cl, color='dimgrey', linewidth=3, linestyle='dashed')
        ax12.text(0.03, 0.13, r'F\textsubscript{diff} = ' + str(abs(round(F_Cl_i_diff, 3))), horizontalalignment='left', verticalalignment='center', transform=ax12.transAxes, size=16)
        ax12.text(0.03, 0.05, r'F\textsubscript{drift} = ' + str(abs(round(F_Cl_i_drift, 3))), horizontalalignment='left', verticalalignment='center', transform=ax12.transAxes, size=16)
